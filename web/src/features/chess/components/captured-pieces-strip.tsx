...
import { type CapturedPieceType } from '../lib/chess-pieces'
...
export function CapturedPiecesStrip({
  ...
  return (
    <div
      ...
      className='border-border/60 from-background/95 to-muted/45 hover:border-border/90 flex min-h-11 w-full flex-col items-center justify-center rounded-2xl border bg-gradient-to-b px-1.5 py-1.5 shadow-sm transition-colors duration-200'
      role='group'
    >
      <div className='flex w-full [scrollbar-width:none] items-center justify-center gap-1 overflow-x-auto sm:flex-col sm:overflow-visible [&::-webkit-scrollbar]:hidden'>
        {pieces.length === 0 ? (
          <span className='text-muted-foreground/70 text-[9px] leading-none font-medium'>
            --
          </span>
        ) : (
